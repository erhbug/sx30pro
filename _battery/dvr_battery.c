...
void vGet_Volts_System(void){
	...
	P0|= (1<<5);
	SARCON = 0x09; //ADC inicia
		if(!(SARCON & 0x04))
		{
			SARCON |= 0x04;
			while(SARCON & 0x04) //Espera conversion
			{
			}
		}
	iValue_Adc = SARDATA;
	SARCON &= 0xf7;//ADC desactivado

	fVoltage_Conector = (float)(iValue_Adc)*(3.3/255)+0.15;
	fVoltage_Battery = (float)(iValue_Adc)*(3.3/255);
	LCD_GLASS_Float(fVoltage_Conector, 2, LCD_PESO);
...
void vVbatHigh(void){
	...
	iLCD_GLASS_Init();//LCD_Cmd(ENABLE);		/* Habilita el modulo LCD */	
	iLCD_GLASS_Clear();
